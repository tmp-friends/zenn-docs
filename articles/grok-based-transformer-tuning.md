---
title: "Grok-based Transformer の構成要素を取り入れて Two-Tower モデルを改善する"
emoji: "🐦‍🔥"
type: "tech"
topics: ["ai", "機械学習", "レコメンド", "transformer", "pytorch"]
published: false
...
RMSNorm[^2] は、LayerNorm から平均の引き算（リセントリング）を省略し、**二乗平均平方根（RMS）のみで正規化（リスケーリング）する手法**です。
...
RoPE[^3] は、**トークンの位置情報を回転行列として Query と Key に埋め込む手法**です。
...
これを実数の行列形式で書くと、$2 \times 2$ 回転行列になります。
...
SwiGLU[^4] は、Transformer の FFN を**ゲート機構で拡張した手法**です。
...
用いている行動履歴の系列長が256と比較的長いこともあり、「何ステップ前のアクションか」という相対位置情報が効いていると考えられます。絶対位置エンコーディングでは捉えにくい、直近のアクションと過去のアクションの関係性をうまくモデリングできているのではないかと思います。
