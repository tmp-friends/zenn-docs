...
韓国・済州島で開催された KDD 2026 (The 32nd ACM SIGKDD Conference on Knowledge Discovery and Data Mining)[^1] に参加してきました。
...
そこで用いられる方法の一つが Semantic ID です。1アイテムを1トークンで表現するのではなく、複数の小さなコードの組み合わせとして表現します。
...
例えば Item A と Item B は一部のコードを共有しており、意味的に近いアイテムであることを表現できます。小さな Vocabulary の組み合わせで大きなカタログを表現できるため、Vocabulary Size を抑えながらアイテム間の関係もモデルに取り込める点が特徴です。
...
1. 複数 Surface を横断した Pre-training と Surface-specific Fine-tuning
   Home Feed、Search、Related Pins など、Pinterest 内の複数 Surface で発生したユーザ行動を時系列にまとめ、Next-item Prediction によって共通モデルを事前学習します。
...
3. Dense Embedding の自己回帰的生成
   TIGER などの Generative Recommendation では、アイテムを Semantic ID と呼ばれる離散 Token へ変換し、その Token Sequence を生成する方法が提案されています。
   ...
   そこで PinRec では、Transformer が生成した Dense Embedding を Query として Faiss による Approximate Nearest Neighbor Search を行い、実在するアイテムを取得します。

   この構成では Item ID 自体を生成しないため、存在しないアイテムを生成する Hallucination の問題も避けられます。

<!-- 図の差し込み：Figure 1／arXiv:2504.10507・PDFの1ページ
Pinterest 内の複数 Surface の行動系列を PinRec で扱い、Surface や Outcome に応じた推薦を行う全体像を示す図を挿入。
キャプション案：Home Feed、Search、Related Pins など複数 Surface のユーザ行動系列を統一的に学習する PinRec。Surface や目的とする Outcome を Conditioning しながら推薦候補を生成する（Botta et al., Figure 1 より引用）。 -->
...
実運用を意識した Serving についても検証されています。
...
Generative Recommendation というと、Semantic ID を Token として生成する方式をイメージしやすいですが、PinRec は「何を生成するか」には複数の選択肢があることを示している点が印象的でした。

PinRec が生成するのは Item ID や Semantic ID ではなく、従来の Retrieval System でも利用されてきた Dense Embedding です。

つまり、「Transformer によるユーザ行動系列のモデリング」と「Embedding + ANN による既存の Retrieval」といった構成になっています。

特に実サービスへの導入を考えると、この構成は現実的だと感じました。

Generative Recommendation を導入するために Retrieval System 全体を Semantic ID ベースへ置き換える必要はなく、既存の ANN Index や後段の Ranking System を残しながら、Candidate Generation の部分を Generative Model へ置き換えられます。

また、複数 Surface のデータをまとめて Pre-training した後に、Surface ごとのデータで Fine-tuning する構成も興味深いです。

すべての推薦面を完全に一つのモデルへ統合するのではなく、「複数の推薦面に共通するユーザの興味を学習する部分」と「各推薦面の目的へ適応する部分」を分けることで、横断的なデータ活用と Surface 固有の最適化を両立しています。

さらに Outcome Conditioning によって、Save や Click といった目的ごとに異なる候補を生成し、推論時の Retrieval Budget によってそのバランスを調整できる点も、実サービスでは重要だと思います。

PinRec を見ると、Generative Recommendation の価値は単に Two-Tower より高い Recall を実現することだけではなく、長いユーザ行動系列、推薦 Surface、最適化したい Outcome といった複数の情報を一つの Sequence Model に取り込み、Candidate Generation 自体を柔軟に制御できることにあると感じました。

### A Unified Model for Personalization: Language-Steerable Generative Recommendation, Search, and User Understanding [^4]
...
アイテムは、コンテンツ Embedding を量子化して得られる Semantic ID（SID）と呼ばれる短い Token Sequence に変換します。これにより、
...
<!-- 図の差し込み：Figure 1／arXiv:2603.17533v1・PDFの2ページ
行動履歴・自然言語の指示・Semantic ID の関係を示す概要図を挿入。
キャプション案：自然言語とアイテムの Semantic ID を共通の Token Sequence として扱う NEO。自然言語による指示に応じて、推薦・検索・ユーザ理解など複数のタスクを実行する（De Nadai et al., Figure 1 より引用）。 -->
...
1,000万件以上のアイテムを対象としたオフライン評価では、単一の Multi-task NEO が複数の Task-specific Baseline を上回りました。例えば Episode Recommendation では HR@10 が 58%、NDCG@10 が 80%、Audiobook Recommendation では HR@10 が 46%、NDCG@10 が 97% Baseline から改善しています。
...
また、レコメンドと検索が Semantic ID と自然言語という共通の Interface を介してつながっている点も興味深いです。今後 Generative Recommendation が発展すると、検索・推薦・ユーザ理解を個別のシステムとして最適化するのではなく、一つの Personalization Model の異なる Task として扱う方向へ進む可能性があります。

一方、扱える Task が増えるほど評価も複雑になります。カタログに存在するアイテムを正しく生成できるかだけでなく、推薦としての Relevance、ユーザ指示への追従、生成された説明と推薦結果の整合性、ユーザ理解の妥当性などを、それぞれ分けて評価する必要があると感じました。
...
![OnePiece の全体構成](/images/kdd-2026-report/kdd_onepiece.png)
*OnePiece の全体構成*
...
<!-- 図の差し込み：Figure 1／arXiv:2607.25366v1・PDFの2ページ
ドメイン間の相性、統合 Weight、ドメイン数、Parameter Distribution を比較した4パネルの図を挿入。
キャプション案：Model Merging による Cross-domain Recommendation で生じる Negative Transfer と Performance Saturation。異質なドメイン間の知識干渉と、統合するドメイン数を増やした際の性能飽和を示している（Ji et al., Figure 1 より引用）。 -->
...
### The Pitfall of Scaling Up: Uncovering and Mitigating Popularity Bias Amplification in Scaling Transformer-based Recommenders [^7]
...
<!-- 図の差し込み：Figure 1／arXiv:2606.21911v1・PDFの2ページ
モデル規模・層数に対する推薦精度と Long-tail Exposure の変化を示す図を挿入。
キャプション案：Transformer-based Recommender の Scaling に伴う推薦精度と Popularity Bias の変化。Fair-0.95 は、人気度順で下位95%に属するアイテムが Top-1 Recommendation に占める割合を表す（Yang et al., Figure 1 より引用）。 -->
...
[^4]: A Unified Model for Personalization: Language-Steerable Generative Recommendation, Search, and User Understanding: https://research.atspotify.com/publications/unified-model-personalization-language-steerable-generative-recommendation-search-user-understanding
...
[^7]: The Pitfall of Scaling Up: Uncovering and Mitigating Popularity Bias Amplification in Scaling Transformer-based Recommenders: https://arxiv.org/abs/2606.21911
